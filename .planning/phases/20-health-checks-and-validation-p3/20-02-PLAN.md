...
5. Function signature check: grep -q "export async function verifyTriggerSetup" src/cli/setup-trigger.ts
6. CLI command check: grep -q "trigger.dev.*whoami" src/cli/setup-trigger.ts
7. Masking check: grep -q "maskApiKey" src/cli/setup-trigger.ts
8. Syntax check: bun build src/cli/setup-trigger.ts --outdir /tmp 2>&1 | head -20
</verify>
  <done>
