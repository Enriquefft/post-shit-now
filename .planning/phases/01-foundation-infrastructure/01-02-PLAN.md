...
Purpose: Project-scoped keys (starting with napi_re4y...) cannot create projects, but setup wizard only discovers this after attempting database creation. Validate key type early and provide clear guidance.
...
<task type="auto">
  ...
  <action>
In setup-db.ts, after loading NEON_API_KEY and before creating Neon project, call validateNeonApiKey:
