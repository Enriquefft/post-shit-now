...
must_haves:
  truths:
    - "Image providers (GPT Image, Ideogram, Flux) accept db + hubId parameters"
    - "All image providers use getApiKey() for key lookup"
    - "No process.env key reads in image provider files"
  ...
  artifacts:
    ...
    - path: "src/media/image-gen.ts"
      ...
      changes: "Updated ImageProvider.generate() signature to accept optional db + hubId"
  key_links:
...
- With: const apiKey = db && hubId ? await getApiKey(db, hubId, "openai") : process.env.OPENAI_API_KEY;
...
- Remove process.env.FAL_KEY and process.env.IDEOGRAM_API_KEY fallbacks
...
grep -n "process\\.env\\.\\(OPENAI\\|FAL\\|IDEOGRAM\\)_API_KEY" src/media/providers/gpt-image.ts src/media/providers/ideogram.ts src/media/providers/flux.ts | wc -l returns 0
  ...
  <done>
All 3 image providers (GPT Image, Ideogram, Flux) accept optional db + hubId parameters
All API key lookups use getApiKey() when db + hubId provided
ImageProvider interface updated to accept db + hubId
  </done>
</task>
...
<verification>
1. ImageProvider.generate() signature includes optional db and hubId parameters
2. All image providers use getApiKey() when db + hubId provided
3. Conditional fallback allows standalone CLI invocation (when db + hubId undefined)
4. Zero process.env key reads in image provider files (except optional fallback)
