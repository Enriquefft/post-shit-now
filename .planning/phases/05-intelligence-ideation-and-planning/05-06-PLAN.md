...
<objective>
Build the weekly planning engine: calendar state visualization, trend-informed ideation mixed with idea bank, series-first slot allocation with pillar/archetype balancing, per-slot language suggestions, content remixing/recycling, and the /psn:plan multi-phase slash command. Also wire bilingual support into /psn:post (language selection step + "both" two-pass generation) and surface due series episodes in /psn:post's topic-gathering step.

Purpose: This is the capstone of Phase 5 -- the planning command that ties intelligence, ideas, series, and content generation into a coherent weekly workflow. Users go from zero to a fully planned and partially drafted week in one session. The /psn:post updates ensure bilingual and series features work outside the planning context too.
Output: Complete src/planning/ directory, plan CLI, /psn:plan slash command, updated /psn:post and generate.ts for bilingual + series wiring.
</objective>
...
<task type="auto">
  <name>Task 3: Wire bilingual and series features into /psn:post and generate.ts</name>
  <files>
    .claude/commands/psn/post.md
    src/content/generate.ts
  </files>
...
This ensures bilingual posts are independently crafted (per user decision in CONTEXT.md), not translations.
  ...
  <verify>
Run `bun run check` to confirm generate.ts compiles with the new "both" language type.
