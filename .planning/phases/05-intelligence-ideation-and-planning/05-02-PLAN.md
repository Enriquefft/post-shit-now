...
Export a unified `searchAll(query: string): Promise<SearchResult[]>` from a search/index.ts that calls all available providers and merges results.
...
- If no monitored accounts or no X token, return empty array
  ...
  <verify>Run `bun run check` to confirm all files compile. Grep for collectTrends, searchPerplexity, searchExa, searchTavily, searchBrave, checkCompetitors exports.</verify>
  <done>Collector orchestrator aggregates all sources with BYOK degradation. Four search clients exist. Competitive intelligence module queries monitored accounts. All gracefully handle missing credentials.</done>
</task>
...
- src/intelligence/search/ has all 4 search client files
- Each source adapter exports a named function
- Collector returns RawTrend[] and handles missing API keys gracefully
</verification>

<success_criteria>
Complete intelligence collection layer that can gather trends from 6 sources, score them by pillar relevance, search 4 providers on demand, and monitor competitors. All BYOK sources degrade gracefully when keys are missing.
</success_criteria>
