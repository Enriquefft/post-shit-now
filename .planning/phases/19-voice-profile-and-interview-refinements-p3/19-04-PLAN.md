...
wave: 2
depends_on: [19-01]
files_modified: [src/voice/interview.ts, src/voice/types.ts, .claude/commands/psn/voice.md]
autonomous: true
requirements: [m7]

must_haves:
  truths:
    - "Platform-specific persona questions guide users to configure unique voices per platform"
    - "Each platform question includes platform-specific guidance and examples"
    - "Users understand platform-specific norms (hashtags, emojis, formats)"
  artifacts:
...
<objective>
Design platform persona interview questions to help users configure unique voices for each platform (X, LinkedIn, Instagram, TikTok).

Purpose: Ensure content is tailored to each platform's norms (hashtags, emojis, format preferences, tone).
Output: Platform-specific interview questions with guidance and examples
</objective>
...
@src/voice/interview.ts (PLATFORM_QUESTIONS and interview flow)
...
4. Complete interview and check generated profile has platforms.x configured
5. Re-run with multiple platforms selected, verify all platform questions appear
</verification>
...
- Platform personas correctly integrated into VoiceProfile
