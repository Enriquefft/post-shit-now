...
Create the Arctic-based OAuth 2.0 PKCE module. Import from `arctic` package (already will be installed: `bun add arctic` before this task runs).
...
7. Export all schemas, types, and error classes.
  ...
  <verify>Run `bun run check` — no type errors. Verify arctic imports resolve by checking `bun add arctic` was run first (executor should run install before this task).</verify>
