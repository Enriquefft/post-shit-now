	...
	for (const post of postsToCollect) {
		try {
			...
			if (isThread) {
				...
				if (!firstId) continue;
				const tweet = tweetMetricsMap.get(firstId);
			...
			const firstTweet = tweetIds[0] ? tweetMetricsMap.get(tweetIds[0]) : undefined;
			...
			await db
				...
				.values({
					...
					postId: post.id,
					platform: "x",
					externalPostId: post.externalPostId ?? tweetIds[0] ?? post.id,
