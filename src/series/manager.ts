...
export async function createSeries(db: HubDb, userId: string, input: CreateSeriesInput) {
	const rows = await db
		.insert(series)
		.values({
			userId,
...
export async function updateSeries(
	...
	if (updates.trackingMode !== undefined) setClause.trackingMode = updates.trackingMode;
	if (updates.trackingFormat !== undefined) setClause.trackingFormat = updates.trackingFormat;
	...
	const rows = await db.update(series).set(setClause).where(eq(series.id, seriesId)).returning();
...
export async function pauseSeries(db: HubDb, seriesId: string) {
	...
	if (existing.status === "retired") throw new Error("Cannot pause a retired series");
	...
	if (!updatedSeries) {
		throw new Error(`Failed to update series: ${seriesId}`);
	}
...
export async function resumeSeries(db: HubDb, seriesId: string) {
	const [existing] = await db.select().from(series).where(eq(series.id, seriesId)).limit(1);
	if (!existing) throw new Error(`Series not found: ${seriesId}`);
	if (existing.status !== "paused") throw new Error("Can only resume a paused series");
...
export async function retireSeries(db: HubDb, seriesId: string) {
	const [existing] = await db.select().from(series).where(eq(series.id, seriesId)).limit(1);
	if (!existing) throw new Error(`Series not found: ${seriesId}`);
	if (existing.status === "retired") throw new Error("Series is already retired");
...
export async function getSeriesAnalytics(
	...
	const avgEngagement =
		totalEpisodes > 0
			? Math.round(metricsRows.reduce((sum, r) => sum + r.engagementScore, 0) / totalEpisodes)
			: 0;

	// Find latest episode date
	const dates = metricsRows.map((r) => r.publishedAt).filter((d): d is Date => d !== null);
